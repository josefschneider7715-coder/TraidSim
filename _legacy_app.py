...
with documentation_tab:
    documentation = {
        "de": """### Benutzerhandbuch\n1. Watchlist und Zeitraum links wählen.\n2. In **Übersicht** Signale und Backtest prüfen.\n3. Mit **Simulation** Kriterien testen.\n4. In **Hyperopt** Ziel wählen und Optimierung starten.\n\n### Architektur\n\n#### 1. Benutzeroberfläche und Anmeldung\nStreamlit rendert die Anmeldung, Seitennavigation, Eingabefelder, Tabellen und Diagramme. `app.py` ist der Einstiegspunkt; `_legacy_app.py` enthält den Hauptablauf und die mehrsprachige Oberfläche. Zugangsdaten werden ausschließlich aus den Streamlit-Secrets gelesen.\n\n#### 2. Marktdaten\n`src/data_provider.py` lädt Aktienkurse über Yahoo Finance und unterstützte Kryptowährungen über Binance. Alle Quellen werden in dasselbe OHLCV-Format mit Datum, Open, High, Low, Close und Volumen überführt.\n\n#### 3. Indikatoren und Parameter\n`src/indicators.py` berechnet SMA, RSI, MACD, Bollinger, Fibonacci, Volumen, Stochastik, ATR und Ichimoku. Perioden, Schwellen und Faktoren werden als typisierte Parameter übergeben. Dadurch verwenden Hyperopt und Simulation dieselben Berechnungen.\n\n#### 4. Signal-Engine\n`src/strategy.py` und `src/telemetry.py` prüfen die aktivierten Kriterien. Ein Einstieg wird nur erzeugt, wenn alle ausgewählten technischen Kriterien erfüllt sind. Das Risikomanagement ist kein Kursfilter: Es bestimmt Positionsgröße, ATR-Stop-Loss und ATR-Take-Profit.\n\n#### 5. Backtest\n`src/backtest.py` verarbeitet die Signale chronologisch, berücksichtigt Gebühren und simuliert Ein- und Ausstiege. Als Ergebnis entstehen Trade-Liste, Kapitalverlauf, Rendite, Drawdown, Trefferquote, Anzahl der Trades und Endkapital.\n\n#### 6. Hyperopt\nHyperopt testet viele Kombinationen aus Kriterien und Parameterwerten. Jeder Durchlauf verwendet dieselbe Indikator-, Signal- und Backtest-Logik wie die Simulation. Anschließend werden Konvergenz, beste Konfiguration, Importance, Heatmap, Sensitivität, Stabilität sowie Vergleiche mit Buy-and-Hold und Oracle erzeugt.\n\n#### 7. Simulation und Telemetrie\nDie Simulation übernimmt auf Wunsch die vollständige Hyperopt-Empfehlung. Sie wertet zusätzlich aus, welche Kriterien häufig erfüllt, blockierend, unterstützend oder entscheidend waren und erstellt daraus Ranglisten sowie Wochen- und Monatsansichten.\n\n#### 8. Speicherung, Visualisierung und Veröffentlichung\n`src/storage.py` speichert Watchlists, Signalhistorie und Alarme in SQLite. Plotly erstellt die interaktiven Diagramme. Änderungen auf GitHub `main` werden von Streamlit Community Cloud übernommen und über `traisim.streamlit.app` veröffentlicht.\n\n**Datenfluss:** Marktdaten → Normalisierung → Indikatoren → Kriterien → Signale → Backtest → Kennzahlen → Hyperopt/Simulation → Tabellen und Diagramme.\n\n### Roadmap\nWalk-forward-Validierung, persistente Optimierungsläufe, weitere Datenanbieter und Benachrichtigungen.\n\n### Changelog\n- Hyperopt mit Mehrzieloptimierung, Importance, Heatmaps, Sensitivität und Stabilitätsindex\n- Dokumentations-Reiter\n- Sprachauswahl Deutsch/Englisch/Russisch\n\n### API und Kontext-Hilfe\nDie Kernfunktionen liegen in `src/`. Tooltips und Erläuterungen befinden sich unmittelbar an den Eingaben.\n\n### Ideen\nPortfolio-Optimierung, Out-of-sample-Ranglisten und automatische Strategieberichte.""",
        "en": """### User guide\n1. Select watchlist and period in the sidebar.\n2. Review signals and backtest in **Overview**.\n3. Test criteria in **Simulation**.\n4. Select an objective in **Hyperopt** and run it.\n\n### Architecture\n\n#### 1. User interface and authentication\nStreamlit renders authentication, navigation, inputs, tables and charts. `app.py` is the entry point; `_legacy_app.py` contains the main workflow and multilingual interface. Credentials are read exclusively from Streamlit secrets.\n\n#### 2. Market data\n`src/data_provider.py` loads stocks from Yahoo Finance and supported cryptocurrencies from Binance. Every source is normalized to the same OHLCV structure containing date, open, high, low, close and volume.\n\n#### 3. Indicators and parameters\n`src/indicators.py` calculates SMA, RSI, MACD, Bollinger, Fibonacci, volume, stochastic, ATR and Ichimoku. Periods, thresholds and factors are passed as typed parameters, so Hyperopt and Simulation use identical calculations.\n\n#### 4. Signal engine\n`src/strategy.py` and `src/telemetry.py` evaluate the enabled criteria. An entry is produced only when all selected technical criteria pass. Risk management is not a price filter; it controls position size, ATR stop-loss and ATR take-profit.\n\n#### 5. Backtest\n`src/backtest.py` processes signals chronologically, includes fees and simulates entries and exits. It produces trades, equity curve, return, drawdown, win rate, completed-trade count and final capital.\n\n#### 6. Hyperopt\nHyperopt tests many combinations of criteria and parameter values. Every trial uses the same indicator, signal and backtest logic as Simulation. It then produces convergence, the best configuration, importance, heatmap, sensitivity, stability and Buy-and-Hold/Oracle comparisons.\n\n#### 7. Simulation and telemetry\nSimulation can import the complete Hyperopt recommendation. It also records which criteria passed, blocked, supported or decided signals and derives rankings plus weekly and monthly views.\n\n#### 8. Storage, visualization and deployment\n`src/storage.py` stores watchlists, signal history and alerts in SQLite. Plotly creates interactive charts. Commits on GitHub `main` are deployed by Streamlit Community Cloud to `traisim.streamlit.app`.\n\n**Data flow:** Market data → normalization → indicators → criteria → signals → backtest → metrics → Hyperopt/Simulation → tables and charts.\n\n### Roadmap\nWalk-forward validation, persistent optimization runs, additional providers and notifications.\n\n### Changelog\n- Hyperopt with multi-objective scoring, importance, heatmaps, sensitivity and stability index\n- Documentation tab\n- German/English/Russian language selector\n\n### API and context help\nCore functions live in `src/`; explanations are shown next to relevant controls.""",
        "ru": """### Руководство\n1. Выберите список инструментов и период слева.\n2. Проверьте сигналы и бэктест в **Обзоре**.\n3. Тестируйте критерии в **Симуляции**.\n4. Выберите цель в **Hyperopt** и запустите оптимизацию.\n\n### Архитектура\n\n#### 1. Интерфейс и аутентификация\nStreamlit отображает вход, навигацию, поля ввода, таблицы и графики. `app.py` является точкой входа, а `_legacy_app.py` содержит основной процесс и многоязычный интерфейс. Учётные данные читаются только из Streamlit Secrets.\n\n#### 2. Рыночные данные\n`src/data_provider.py` загружает акции через Yahoo Finance, а поддерживаемые криптовалюты — через Binance. Все источники приводятся к единому формату OHLCV: дата, открытие, максимум, минимум, закрытие и объём.\n\n#### 3. Индикаторы и параметры\n`src/indicators.py` рассчитывает SMA, RSI, MACD, Bollinger, Fibonacci, объём, стохастик, ATR и Ichimoku. Периоды, пороги и коэффициенты передаются как типизированные параметры, поэтому Hyperopt и Симуляция используют одинаковые расчёты.\n\n#### 4. Модуль сигналов\n`src/strategy.py` и `src/telemetry.py` проверяют активные критерии. Сигнал входа создаётся только при выполнении всех выбранных технических критериев. Управление риском не является ценовым фильтром: оно задаёт размер позиции, ATR stop-loss и ATR take-profit.\n\n#### 5. Бэктест\n`src/backtest.py` последовательно обрабатывает сигналы, учитывает комиссии и моделирует входы и выходы. Результаты включают сделки, кривую капитала, доходность, просадку, долю прибыльных сделок, число сделок и конечный капитал.\n\n#### 6. Hyperopt\nHyperopt проверяет множество комбинаций критериев и параметров. Каждый запуск использует ту же логику индикаторов, сигналов и бэктеста, что и Симуляция. Затем рассчитываются сходимость, лучшая конфигурация, важность, тепловая карта, чувствительность, стабильность и сравнения с Buy-and-Hold и Oracle.\n\n#### 7. Симуляция и телеметрия\nСимуляция может полностью принять рекомендацию Hyperopt. Она также определяет, какие критерии выполнялись, блокировали, поддерживали или определяли сигналы, и формирует рейтинги, недельные и месячные представления.\n\n#### 8. Хранение, визуализация и публикация\n`src/storage.py` хранит списки, историю сигналов и оповещения в SQLite. Plotly создаёт интерактивные диаграммы. Изменения ветки GitHub `main` публикуются Streamlit Community Cloud на `traisim.streamlit.app`.\n\n**Поток данных:** Рыночные данные → нормализация → индикаторы → критерии → сигналы → бэктест → метрики → Hyperopt/Симуляция → таблицы и графики.\n\n### План\nWalk-forward проверка, сохранение запусков, новые источники данных и уведомления.\n\n### Изменения\n- Hyperopt: несколько целей, важность, тепловые карты, чувствительность и стабильность\n- Вкладка документации\n- Выбор немецкого, английского и русского языков.""",
    }
